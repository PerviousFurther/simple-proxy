...
struct artist
{
	...
	{
		::std::cout << "hello world. with " << ::std::string_view{string} << ::std::endl;
		return 0;
	}
	inline void dance()
	{
		::std::cout << "hello world in dance.";
	}
};
...
struct prx::make_t<unique_ptr<T>>
{
	...
	{ return {new T{::std::forward<Args>(args)...}, dynamic_deleter{prx::inplace<T>{}}}; }
};

int main() 
{
	prx::proxy<unique_ptr, prx::entity<draw, prx::primitive_traits<dance, void()>>> proxy{};
	proxy.construct<artist>();
	proxy.invoke<draw>();
	proxy.invoke<draw>(6);
	::std::cout << "some say: " << proxy.invoke<draw>("having value of:") << ::std::endl;
	proxy.invoke<dance>();
}