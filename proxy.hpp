...
{
  using type = typename find<by_index,
                             ::std::integral_constant<T, value - 1>,
                             Tl<Ts...>>::type;
};
template<::std::integral T,
         typename F,
...
using element_t =
  typename meta::find<meta::by_index, meta::csize<index>, types<Ts...>>::type;
template<::std::size_t index, typename T>
using element_at =
  typename meta::find<meta::by_index, meta::csize<index>, T>::type;
...
struct add
{
  ...
  static constexpr auto value{ T::value == size_max ? T::value
                                                    : T::value + increment };
};
...
struct find<first_of, Pred<Tf, void>, Tl<F, Rs...>>
{
  ...
  using next = find<first_of, Pred<Tf, void>, Tl<Rs...>>;
  using conditional =
    typename ::std::conditional_t<result, ::std::type_identity<F>, next>;

  using type = typename conditional::type;
  static constexpr auto is_contain{
    !::std::is_void_v<typename conditional::type>
  };
  static constexpr auto value{
    ::std::conditional_t<result,
...
struct find<most_of, Pred<Mst, void>, Tl<F, Rs...>>
{
  ...
  using next =
    find<most_of,
         ::std::conditional_t<result, Pred<F, void>, Pred<Mst, void>>,
         Tl<Rs...>>;
...
concept presented_in = meta::
  find<meta::first_of, ::std::is_same<TF, void>, types<Ts...>>::is_contain;
template<typename TF, typename T>
concept presented_at =
  meta::find<meta::first_of, ::std::is_same<TF, void>, T>::is_contain;
template<typename TF, typename... Ts>
inline constexpr auto index_of =
  meta::find<meta::first_of, ::std::is_same<TF, void>, types<Ts...>>::value;
template<typename TF,
         template<typename, typename>
         typename Pred,
         typename... Ts>
using first_required_t =
  typename meta::find<meta::first_of, Pred<TF, void>, types<Ts...>>::type;

inline constexpr struct no_return_t final
{
} no_return{};
} // namespace prx::inner
...
struct primitive_traits<B, R(Ps...)>
{
  ...
  template<typename I>
  static constexpr auto represented()
  {
    return ::std::invocable<B, I&, Ps...>;
  };
};
...
struct primitive_traits<B, Fs...> : primitive_traits<B, Fs>...
{
  ...
  template<typename H>
  static consteval auto represented()
  {
    return ((single<Fs>::template represented<H>()) && ...);
  };
};
...
#define PR_X_CONSTEXPR_ constexpr
    ...
    {
      inline static PR_X_CONSTEXPR_ ::std::
        conditional_t<::std::is_void_v<R>, inner::no_return_t, R> (
          *value)(void*, Ps...){ &do_invoke<F, R, Ps...> };
    };
    ...
    static PR_X_CONSTEVAL_ auto single()
    {
      if constexpr (overloaded_primitive<F>)
        return (void* const)
          single_impl<F, typename F::template index_t<index>>::value;
      else if constexpr (simple_primitive<F>)
        return (void* const)single_impl<F, typename F::type>::value;
      else
        static_assert(::std::is_void_v<F> ? false : false,
                      "Proxy error: not required.");
    }
    ...
    template<::std::size_t index = 0,
             ::std::size_t size = 0,
             ::std::size_t... indices>
    static PR_X_CONSTEVAL_ auto make(
      const array_t<size> prev = {},
      ::std::index_sequence<indices...> = ::std::make_index_sequence<0>{})
    {
      if constexpr (index >= (sizeof...(Rs)))
        ...
        auto current{ overloaded<this_f, new_size, size>(
          prev,
...
private:
  ...
  static consteval auto located() noexcept
  {
    if constexpr (index != end)
      return located<end,
                     index + 1u,
                     offset + inner::element_t<index, Rs...>::size>();
    else
      return offset;
  }
...
public:
  ...
  inline auto invoke(void* data, Args&&... args) const
  {
    ...
    if constexpr (simple_primitive<F>) {
      ...
      static_assert(::std::invocable<type, Args...>,
                    "[PRX ERROR]: primitive is not included.");
    } else // overloaded.
    {
      if constexpr (index < F::size) {
        ...
        if constexpr (::std::invocable<type, Args...>)
          ...
          return this->invoke<F, index + 1u>(data,
                                             ::std::forward<Args>(args)...);
      }
      static_assert(index != F::size,
                    "[PRX ERROR]: primitive is not included.");
    }
  }
...
template<typename T>
concept entity =
  is_entity<T>::value; // all of requirment should be functionality.

template<template<typename> typename PtrM, typename Pm>
struct proxy
{
  static_assert(::std::is_void_v<Pm> ? false : false,
                "[PRX ERROR]: Not an valid proxy.");
};

template<typename T>
struct make_t;

template<template<typename> typename PtrM,
         template<typename...>
         typename Ett,
         primitive... Ps>
...
struct proxy<PtrM, Ett<Ps...>>
{
  ...
  template<template<typename> typename OPtrM,
           template<typename...>
           typename OEtt>
  constexpr proxy(proxy<OPtrM, OEtt<Ps...>>&& other)
    requires(::std::convertible_to<OPtrM<void>, PtrM<void>> &&
             ::std::convertible_to<OEtt<Ps...>, Ett<Ps...>>)
  ...
  explicit constexpr proxy(inplace<Ent> use_prx_inplace, Args&&... args)
    requires(::std::constructible_from<Ent, Args && ...> &&
             requires { sizeof(make_t<PtrM<Ent>>); })
...
public:
  ...
  {
    if (!value_.get())
      throw bad_proxy_call{};
    if constexpr (inner::presented_at<T, entity>)
      return ent_.template invoke<T>(value_.get(),
                                     ::std::forward<Args>(args)...);
    else if constexpr (inner::presented_in<T, typename Ps::name_t...>)
      return ent_.template invoke<inner::first_required_t<T, find, Ps...>>(
        value_.get(), ::std::forward<Args>(args)...);
    else
      static_assert(inner::presented_in<T, typename Ps::name_t...>,
                    "[PRX ERROR]: Primitive not prsented.");
  }

  template<typename Ent, typename... Args>
    requires ::std::constructible_from<Ent, Args&&...>
  inline auto construct(Args&&... args)
    requires requires { sizeof(make_t<PtrM<Ent>>); }
  {
  ...
  friend inline constexpr this_t make(Args&&... args)
    requires(::std::same_as<T, this_t> &&
             ::std::constructible_from<this_t, Args && ...>)
  {
